...
* 인증번호 생성
    ...
    * 사용자 휴대폰 검증 
        * 예외
    * 인증번호와 휴대폰 번호 맵핑해서 저장 
        * 인증번호를 발급한 휴대폰번호일 때
            * 저장소에 휴대폰번호를 찾아 새로운 인증코드 업데이트
        * 최초 인증번호 발급일 때
            * 저장소에 휴대폰 번호, 인증번호 맵핑해서 저장
* 5분 초과시 만료 검증 
    * 인증코드 생성 시간 + 5분, 만료시간 검증시 현재 시간과 비교
    * 간단하게 전역 map으로 구현 -> session
* request: 휴대폰 번호
* response: 인증번호
#### 예외처리
    * 휴대폰 번호 양식
...
* 인증번호 양식 검증 
    ...
    * 제한시간 만료 확인
        * 만료 예외
    * 인증번호 틀리거나 존재하지 않으면 예외
    * 인증 완료시 삭제
* request: 휴대폰 번호, 인증번호
* response: 200 or 400
#### 예외처리
    * 인증번호가 틀리거나 존재하지 않을 경우
    * 인증번호 양식
    * 인정번호 만료
